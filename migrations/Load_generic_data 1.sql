...
def move_invalid_rows_to_error(session, table_full, error_table_full):
    ...
    for r in schema_rows:
        ...
        if any(x in typ for x in ['NUMBER', 'DECIMAL', 'INT', 'FLOAT', 'DOUBLE']):
            ...
            invalid_conds.append(col + " IS NULL")
        elif 'VARIANT' in typ or 'OBJECT' in typ or 'ARRAY' in typ:
            invalid_conds.append(col + " IS NULL")
        else:
...
def ensure_audit_table(session, audit_table_full):
    """Create audit table if not present."""
    if table_exists(session, audit_table_full):
        return True
    sql = (
...
def insert_audit(
    ...
    sql = (
        ...
        " (DATASET_NAME, FILE_NAME, FILE_FORMAT, LOAD_STATUS, ROW_COUNT, ROWS_LOADED, "
...
def csv_two_stage_load(
    ...
    for idx, f in enumerate(files):
        ...
        stg_cols = ", ".join(
            ...
            ['FILE_NAME STRING', 'ROW_NUMBER NUMBER']
        )
        session.sql(
        ...
        select_parts = ", ".join(
            ...
            ["METADATA$FILENAME as FILE_NAME", "METADATA$FILE_ROW_NUMBER as ROW_NUMBER"]
        )
        copy_sql = (
            "COPY INTO " + temp_stg +
        ...
        if row_cnt == 0:
            insert_audit(
                session, audit_table, dataset_name, fname_short, file_format,
        ...
        if invalid_cnt > 0:
            ...
            insert_error_sql = (
                ...
                ) + ", FILE_NAME, 'CSV validation failed', 'CSV_VALIDATION', ROW_NUMBER, " +
                "'[' || " + raw_concat + " || ']' " +
...
def parquet_simple_copy(
    ...
    for r in results:
        ...
        insert_audit(
            session, audit_table, dataset_name, file_name.split('/')[-1],
            file_format, status, rows_parsed, rows_loaded, errors_seen,
    ...
    try:
        ...
        for ds in datasets:
            ...
            try:
                ...
                try:
                    tgt_rows = session.sql(
                        "SELECT COUNT(*) AS CNT FROM " + table_full
                    ).collect()[0]['CNT']
                except Exception:
                    tgt_rows = 0

                try:
                    err_rows = session.sql(
                        "SELECT COUNT(*) AS CNT FROM " + error_table_full
                    ).collect()[0]['CNT']
                except Exception:
                    err_rows = 0
                ...
                insert_audit(
                    ...
                    err_rows,
                    0,
                    "Dataset " + str(dataset_name) + " summary",
                    job_id,
                    "DATASET_SUMMARY",
                    ds_duration,
                    ds_start,
...
SELECT * FROM PRODUCT_MASTER_ERRORS;
DROP TABLE IF EXISTS DRAI_INGESTION_DATABASE.DRAI_ING_SCHEMA.PRODUCT_MASTER;
DROP TABLE IF EXISTS DRAI_INGESTION_DATABASE.DRAI_ING_SCHEMA.PRODUCT_MASTER_ERRORS;
